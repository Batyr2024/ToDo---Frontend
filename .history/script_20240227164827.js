...
function handleAddTask() {
  ...
  listItem.innerHTML = `
      ...
      <input class="checkbox"  name="taskCheckbox" id=${idArry} type ="checkbox">
      <span>${taskText}</span>
...
checkboxSelectAll.addEventListener('click', () => {
  arrayTask.forEach((element, i) => {
    ...
    const virableElement = element;
    if (element.isChecked === false) {
      virableElement.isChecked = true;
      document.getElementById(idCheckbox).checked = true;
    } else {
      virableElement.isChecked = false;
...
taskDeleted.addEventListener('click', () => {
  arrayTask = arrayTask.filter(isChecked === false);
  initElement();
});
