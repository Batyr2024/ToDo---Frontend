...
const checkboxAll = document.getElementsByClassName('checkbox');
...
function checkboxClicksTasks(event) {
  ...
  let checkedAllTrue = null;
  for (let i = 0; i < arrayTask.length; i++) {
    if (element.isChecked === false) { checkedAllTrue = false; break; } else { checkedAllTrue = true; }
  }
  if (arrayTask[index].isChecked === false) {
    if (checkedAllTrue === true) {checkboxSelectAll.checked=}
