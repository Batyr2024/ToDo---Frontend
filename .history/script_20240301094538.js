...
function deleteSelectTasks() {
  let arrayTemp = arrayTask.filter((value) => value.isChecked === false);
  arrayTask.forEach((element) => {
  arrayTask[i].id=arrayTemp[i].id;
  arrayTask[i].text=arrayTemp[i].text;
  arrayTask[i].isChecked=arrayTemp[i].isChecked;
  });
  render();
