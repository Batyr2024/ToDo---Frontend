...
const taskItems = document.getElementsByTagName();
...
function changeTasks(event) {
  ...
  arrayTask.forEach((element) => {
    ...
    <li id=${idCheckbox}>
    <input class="checkbox"  id=${idCheckbox} type ="checkbox">
    <input class="task-List-Input" type="text" value=${taskText}>
    <span id=${idCheckbox} class="delete">X</span>
    </li>
        `;
    } else {
...
function addChangeTasks() {
  const taskListInput = taskList.querySelector(idGlobalArray('.task-List-Input'));
  const taskText = taskListInput.value.trim();
  const index = arrayTask.findIndex((el) => el.id === Number(idGlobalArray));
...
taskList.addEventListener('keyup', addChangeTasks);
