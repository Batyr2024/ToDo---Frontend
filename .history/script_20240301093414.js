...
function handleAddTask(arrayTask) {
...
function selectAllCheckboxTasks(arrayTask) {
...
function checkboxClicksTasks(event, arrayTask) {
  const { target } = event;
...
function deleteSelectTasks(arrayTask) {
  arrayTask = arrayTask.filter((value) => value.isChecked === false);
  render();
  checkboxSelectAll.checked = false;
  return arrayTask;
}
function changeTasks(event, arrayTask) {
  const { target } = event;
...
function addChangeTasks(arrayTask) {
