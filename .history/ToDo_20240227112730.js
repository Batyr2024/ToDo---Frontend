...
const listItem=document.createElement("li" , "id=${idarr}");
    ...
    taskList.onclick= function(event){
        ...
        if(target.getElementsByName=='taskCheckbox'){
            let idelem=element.id;
            let index=arrayTask.indexOf(idelem)
            if(arrayTask[index].isChecked==false){
                arrayTask[index].isChecked=truw;   
        }
        else{
            arrayTask[index].isChecked=t;
